...
class WizardTest < Minitest::Test
  ...
  def test_starts_rested
    # skip
    wizard = Wizard.new("Merlin") # create wizard
    assert wizard.rested? # .rested? returns true
  end

  def test_can_cast_spells
    # skip
    wizard = Wizard.new("Merlin") # create wizard
    assert_equal "MAGIC MISSILE!", wizard.cast # .cast returns "MAGIC MISSILE!"
  end

  def test_gets_tired_after_casting_three_spells
    # skip
    wizard = Wizard.new("Merlin") # create wizard
    wizard.cast # casts spell twice
    wizard.cast
    wizard.rested? # check wizard is rested
    wizard.cast # casts spell
    refute wizard.rested? # check wizard is not rested
  end
